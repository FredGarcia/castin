...
Le front VueJS accessible est livré (`front/index.html`) : tableau de bord
(métriques + ECE brut→calibré + alertes), panneau par champ (Copier, confiance,
suggestions, signalement typé, badge « à vérifier » pour les champs routés, **sélecteur
de section pour les champs `text` ambigus**), overlay AR (`source_spans`/`document`,
qui **suit la section choisie**) et récapitulatif de replay. La calibration isotonique,
le routage par seuil et la désambiguïsation multi-candidats sont en place (voir ci-dessus).
